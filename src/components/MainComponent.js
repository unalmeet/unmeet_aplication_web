    ...
    const home = (
      <Switch>
        <Route
          ...
          component={() => (
            <Home
              credentials={(id, user, email, token) =>
                this.credentialLogin(id, user, email, token)
              }
            />
          )}
        />
    ...
    const protectedComponent = (
      <Switch>
        <UserContext.Provider value={1}>
          <Route
            path={"/calendar/" + this.state.user}
            component={() => <Calendar user={this.state} />}
          />
          <Route
            path={"/inmeeting/" + this.state.user}
            component={() => <InMeeting user={this.state} />}
          />
